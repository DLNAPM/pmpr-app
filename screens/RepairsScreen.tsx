...
const RepairForm: React.FC<{ repair?: Repair; properties: {id: string, name: string}[]; contractors: Contractor[]; onSave: (repair: Omit<Repair, 'id'> | Repair) => void; onCancel: () => void; onAddContractor: (contractor: Omit<Contractor, 'id'>) => Contractor; }> = ({ repair, properties, contractors, onSave, onCancel, onAddContractor }) => {
...
const RepairsScreen: React.FC<RepairsScreenProps> = ({ action, editTarget, onActionDone }) => {
    ...
    const handleSave = (repairData: Omit<Repair, 'id'> | Repair) => { if ('id' in repairData) { updateRepair(repairData); } else { addRepair(repairData); } setIsModalOpen(false); setSelectedRepair(undefined); };
    ...
    return (
        <div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selectedRepair ? "Edit Repair Request" : "Add Repair Request"}>
                <RepairForm repair={selectedRepair} properties={properties.map(p => ({ id: p.id, name: p.name }))} contractors={contractors} onSave={handleSave} onCancel={() => setIsModalOpen(false)} onAddContractor={addContractor} />
            </Modal>
        </div>
    );
};
