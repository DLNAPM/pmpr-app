...
const ReportingScreen: React.FC<ReportingScreenProps> = ({ initialFilter, onFilterApplied, onEditItem }) => {
    ...
    const [filters, setFilters] = useState({
        type: 'all', propertyId: 'all', tenantId: 'all', startDate: '', endDate: '',
    ...
    useEffect(() => {
        if (initialFilter) {
            setFilters(prev => ({ ...prev, status: initialFilter.status || 'all', repairStatus: initialFilter.repairStatus || 'all' }));
            onFilterApplied();
        }
    ...
    const reportData = useMemo(() => {
        ...
        payments.forEach(p => {
            ...
            if (!property) return;
            items.push({ date: new Date(p.year, p.month - 1, 1).toISOString(), propertyName: property.name, tenantName: property.tenants[0]?.name, type: 'Rent', category: 'Rent', billAmount: p.rentBillAmount, paidAmount: p.rentPaidAmount, balance: p.rentBillAmount - p.rentPaidAmount, originalId: p.id });
            p.utilities.forEach(u => items.push({ date: new Date(p.year, p.month - 1, 1).toISOString(), propertyName: property.name, tenantName: property.tenants[0]?.name, type: 'Utility', category: u.category, billAmount: u.billAmount, paidAmount: u.paidAmount, balance: u.billAmount - u.paidAmount, originalId: p.id }));
        });
        repairs.forEach(r => {
            const property = properties.find(prop => prop.id === r.propertyId);
            if (!property) return;
            items.push({ date: r.requestDate, propertyName: property.name, tenantName: property.tenants[0]?.name, type: 'Repair', category: 'Repair', billAmount: r.cost, paidAmount: r.status === RepairStatus.COMPLETE ? r.cost : 0, balance: r.status === RepairStatus.COMPLETE ? 0 : r.cost, repairStatus: r.status, originalId: r.id });
        });
    ...
    const processCsv = (csvText: string) => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const record = headers.reduce((obj: Record<string, any>, header, index) => { const value = values[index]?.trim(); if (header === 'Bill Amount' || header === 'Paid Amount') obj[header] = Number(value) || 0; else obj[header] = value; return obj; }, {}) as CsvRow;
    ...
    const handleConfirmImport = () => {
        ...
        let paymentsAdded = 0; let repairsAdded = 0;
        for (const record of importPreview.validRecords) {
            const property = properties.find(p => p.name === record['Property Name']);
            if (!property) continue;
            const date = new Date(record.Date); const year = date.getFullYear(); const month = date.getMonth() + 1;
            if (record.Type === 'Repair') {
                addRepair({ propertyId: property.id, description: record.Category, cost: Number(record['Bill Amount']), status: Number(record['Paid Amount']) > 0 ? RepairStatus.COMPLETE : RepairStatus.PENDING_REPAIRMEN, requestDate: date.toISOString() });
                repairsAdded++;
            } else {
                 const existing = payments.find(p => p.propertyId === property.id && p.year === year && p.month === month);
                 if (existing) {
                    const updated = { ...existing };
                    if (record.Type === 'Rent') { updated.rentBillAmount = Number(record['Bill Amount']); updated.rentPaidAmount = Number(record['Paid Amount']); } 
                    else {
                        const util = updated.utilities.find(u => u.category === record.Category);
                        if (util) { util.billAmount = Number(record['Bill Amount']); util.paidAmount = Number(record['Paid Amount']); } 
                        else updated.utilities.push({ category: record.Category, billAmount: Number(record['Bill Amount']), paidAmount: Number(record['Paid Amount']) });
                    }
                    updatePayment(updated);
                 } else {
                     addPayment({ propertyId: property.id, year, month, rentBillAmount: record.Type === 'Rent' ? Number(record['Bill Amount']) : 0, rentPaidAmount: record.Type === 'Rent' ? Number(record['Paid Amount']) : 0, utilities: record.Type === 'Utility' ? [{ category: record.Category, billAmount: Number(record['Bill Amount']), paidAmount: Number(record['Paid Amount'])}] : [] });
                 }
                 paymentsAdded++;
            }
        }
    ...
    const handleReconcile = () => {
        const paymentGroups = Object.values(payments.reduce<Record<string, Payment[]>>((acc, p) => { const key = `${p.propertyId}-${p.year}-${p.month}`; if (!acc[key]) acc[key] = []; acc[key].push(p); return acc; }, {})).filter(group => group.length > 1);
        const repairGroups = Object.values(repairs.reduce<Record<string, Repair[]>>((acc, r) => { const key = `${r.propertyId}-${r.description}-${r.cost}-${new Date(r.requestDate).toLocaleDateString()}`; if (!acc[key]) acc[key] = []; acc[key].push(r); return acc; }, {})).filter(group => group.length > 1);
        setDuplicatePayments(paymentGroups);
        setDuplicateRepairs(repairGroups);
        const initialSelections: Record<string, string> = {};
        paymentGroups.forEach((group, index) => { const best = group.sort((a,b) => (b.paymentDate ? 1: -1) - (a.paymentDate ? 1 : -1) )[0]; initialSelections[`payment-${index}`] = best.id; });
        repairGroups.forEach((group, index) => { const best = group.sort((a,b) => (b.completionDate ? 1: -1) - (a.completionDate ? 1 : -1) )[0]; initialSelections[`repair-${index}`] = best.id; });
    ...
    const handleConfirmReconciliation = () => {
        ...
        duplicatePayments.forEach((group, index) => { const keepId = selections[`payment-${index}`]; group.forEach(p => { if (p.id !== keepId) recordsToDelete.push({type: 'payment', id: p.id}) }); });
        duplicateRepairs.forEach((group, index) => { const keepId = selections[`repair-${index}`]; group.forEach(r => { if (r.id !== keepId) recordsToDelete.push({type: 'repair', id: r.id}) }); });
        ...
        <Card>
            <CardHeader>
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleReconcile} disabled={isReadOnly} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:opacity-50"> <ShieldCheckIcon className="w-4 h-4 text-green-600" /> Reconcile </button>
            ...
            <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4 p-4 bg-slate-50 rounded-lg">
                    ...
                    <select name="tenantId" value={filters.tenantId} onChange={handleFilterChange} className="p-2 border rounded"> <option value="all">All Tenants</option> {tenants.map(t => <option key={t.id} value={t.id}>{t.name}</option>)} </select>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {reportData.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm"> <span className={`font-semibold ${item.type === 'Repair' ? 'text-yellow-700' : 'text-blue-700'}`}>{item.type}</span> {item.type !== item.category && <span className="text-gray-500"> / {item.category}</span>} </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                                        {!isReadOnly && (
                                            <button onClick={() => onEditItem({ type: item.type === 'Repair' ? 'repair' : 'payment', id: item.originalId })} className="text-blue-600 hover:text-blue-900">
