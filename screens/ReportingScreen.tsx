            ...
            <CardHeader>
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleReconcile} disabled={authStatus === 'authenticated' && properties.filter(p => isOwner(p.userId)).length === 0} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:bg-gray-200 disabled:cursor-not-allowed"><ShieldCheckIcon className="w-4 h-4 text-green-600" /> Reconcile</button>
                        <button onClick={() => fileInputRef.current?.click()} disabled={authStatus === 'authenticated' && properties.filter(p => isOwner(p.userId)).length === 0} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:bg-gray-200 disabled:cursor-not-allowed"><ArrowUpTrayIcon className="w-4 h-4" /> Import</button>
