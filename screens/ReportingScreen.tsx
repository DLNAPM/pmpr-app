...
const ReportingScreen: React.FC<ReportingScreenProps> = ({ initialFilter, onFilterApplied, onEditItem }) => {
    ...
    const [filters, setFilters] = useState({ type: 'all', propertyId: 'all', tenantId: 'all', startDate: '', endDate: '', status: initialFilter?.status || 'all' as 'all' | 'collected' | 'outstanding', repairStatus: initialFilter?.repairStatus || 'all' as 'all' | 'open' | 'completed', });
    ...
    const isOwner = (itemUserId: string) => authStatus === 'guest' || (user && itemUserId === user.id);
    const userOwnedProperties = useMemo(() => properties.filter(p => isOwner(p.userId)), [properties, isOwner]);

    const tenants = useMemo(() => properties.flatMap(p => p.tenants.map(t => ({...t, propertyName: p.name}))), [properties]);
    ...
    const reportData = useMemo(() => { 
        ...
        payments.forEach(p => { 
            ...
            if (!property) return; 
            // Create a main tenant name string for display
        ...
        return items.filter(item => { 
            ...
            if (filters.tenantId !== 'all') {
                ...
                if (property?.id !== properties.find(p => p.tenants.some(t => t.id === tenant?.id))?.id) return false;
            }
    ...
    return (
        <Card>
            <CardHeader>
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleReconcile} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50"><ShieldCheckIcon className="w-4 h-4 text-green-600" /> Reconcile</button>
                        <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50"><ArrowUpTrayIcon className="w-4 h-4" /> Import</button>
            ...
            <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4 p-4 bg-slate-50 rounded-lg"> <select name="type" value={filters.type} onChange={handleFilterChange} className="p-2 border rounded"> <option value="all">All Types</option> <option value="Rent">Rent</option> <option value="Utility">Utility</option> <option value="Repair">Repair</option> </select> <select name="propertyId" value={filters.propertyId} onChange={handleFilterChange} className="p-2 border rounded"> <option value="all">All Properties</option> {properties.map(p => <option key={p.id} value={p.id}>{p.name}{p.ownerInfo ? ' (Shared)' : ''}</option>)} </select> <select name="tenantId" value={filters.tenantId} onChange={handleFilterChange} className="p-2 border rounded"> <option value="all">All Tenants</option> {tenants.map(t => <option key={t.id} value={t.id}>{t.name} ({t.propertyName})</option>)} </select> <select name="status" value={filters.status} onChange={handleFilterChange} className="p-2 border rounded"> <option value="all">All Status</option> <option value="collected">Collected</option> <option value="outstanding">Outstanding</option> </select> {filters.type === 'Repair' && ( <select name="repairStatus" value={filters.repairStatus} onChange={handleFilterChange} className="p-2 border rounded"> <option value="all">All Repair Status</option> <option value="open">Open</option> <option value="completed">Completed</option> </select> )} <input type="date" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="p-2 border rounded"/> <input type="date" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="p-2 border rounded"/> </div>
