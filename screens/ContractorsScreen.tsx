...
const ContractorForm: React.FC<{ contractor?: Contractor; onSave: (contractor: Omit<Contractor, 'id'> | Contractor) => void; onCancel: () => void; }> = ({ contractor, onSave, onCancel }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); if (formData.name && formData.contact) { onSave(contractor ? { ...formData, id: contractor.id } : formData); } else { alert("Contact Person Name and Phone are required."); } };
...
interface ImportPreview { validRecords: Omit<Contractor, 'id'>[]; errors: { row: number; message: string }[]; }
...
const ContractorsScreen: React.FC = () => {
    ...
    const openAddModal = () => { if(isReadOnly) return; setSelectedContractor(undefined); setIsModalOpen(true); };
    const openEditModal = (contractor: Contractor) => { if(isReadOnly) return; setSelectedContractor(contractor); setIsModalOpen(true); };
    const handleSave = (contractorData: Omit<Contractor, 'id'> | Contractor) => { if ('id' in contractorData && contractorData.id) { updateContractor(contractorData); } else { addContractor(contractorData); } setIsModalOpen(false); setSelectedContractor(undefined); };
    const handleExport = () => { const headers = ['Name', 'Contact', 'CompanyName', 'CompanyAddress', 'Email', 'Comments']; const escapeCsvCell = (cellData: string | undefined) => { if (cellData === undefined || cellData === null) return ''; const str = String(cellData); if (str.includes(',') || str.includes('"') || str.includes('\n')) { return `"${str.replace(/"/g, '""')}"`; } return str; }; const rows = contractors.map(c => [c.name, c.contact, c.companyName, c.companyAddress, c.email, c.comments].map(escapeCsvCell).join(',')); const csvContent = [headers.join(','), ...rows].join('\n'); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement('a'); const url = URL.createObjectURL(blob); link.setAttribute('href', url); const date = new Date().toISOString().split('T')[0]; link.setAttribute('download', `pmpr_contractors_${date}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link); };
    ...
    const processCsv = (csvText: string) => { const lines = csvText.split(/\r\n|\n/).filter(line => line.trim()); if (lines.length < 2) { alert('CSV file must have a header row and at least one data row.'); return; } const headers = lines[0].split(',').map(h => h.trim()); const requiredHeaders = ['Name', 'Contact']; if (!requiredHeaders.every(h => headers.includes(h))) { alert(`CSV is missing required headers. Must include: ${requiredHeaders.join(', ')}`); return; } const preview: ImportPreview = { validRecords: [], errors: [] }; for (let i = 1; i < lines.length; i++) { const values = lines[i].split(','); const rowData: Omit<Contractor, 'id'> = { name: values[headers.indexOf('Name')]?.trim() || '', contact: values[headers.indexOf('Contact')]?.trim() || '', companyName: values[headers.indexOf('CompanyName')]?.trim(), companyAddress: values[headers.indexOf('CompanyAddress')]?.trim(), email: values[headers.indexOf('Email')]?.trim(), comments: values[headers.indexOf('Comments')]?.trim(), }; if (!rowData.name || !rowData.contact) { preview.errors.push({ row: i + 1, message: 'Missing Name or Contact.' }); continue; } preview.validRecords.push(rowData); } setImportPreview(preview); setIsImportModalOpen(true); if(fileInputRef.current) fileInputRef.current.value = ""; };
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Contractors</h2>
                 <div className="flex gap-2">
                    {!isReadOnly && <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50"><ArrowUpTrayIcon className="w-4 h-4" />Import</button>}
                    <input type="file" ref={fileInputRef} onChange={handleFileSelect} accept=".csv" className="hidden"/>
                    <button onClick={handleExport} disabled={isReadOnly} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
