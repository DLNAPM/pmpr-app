...
// Fix: Update onSave to accept a full contractor for updates or a partial for new entries.
const ContractorForm: React.FC<{ contractor?: Contractor; onSave: (contractor: Omit<Contractor, 'id' | 'userId'> | Contractor) => void; onCancel: () => void; }> = ({ contractor, onSave, onCancel }) => {
    ...
    // Fix: Pass the complete contractor object on update to include `id` and `userId`.
...
// Fix: Use correct type for imported contractor records, omitting `userId`.
...
const ContractorsScreen: React.FC = () => {
    ...
    // Fix: Remove `isReadOnly` from `useAuth` as it does not exist and permissions are handled at the data level.
    ...
    // Fix: Update `handleSave` to correctly differentiate between new and existing contractors.
    ...
    const processCsv = (csvText: string) => { const lines = csvText.split(/\r\n|\n/).filter(line => line.trim()); if (lines.length < 2) { alert('CSV file must have a header row and at least one data row.'); return; } const headers = lines[0].split(',').map(h => h.trim()); const requiredHeaders = ['Name', 'Contact']; if (!requiredHeaders.every(h => headers.includes(h))) { alert(`CSV is missing required headers. Must include: ${requiredHeaders.join(', ')}`); return; } const preview: ImportPreview = { validRecords: [], errors: [] }; for (let i = 1; i < lines.length; i++) { const values = lines[i].split(','); 
    // Fix: Use correct type for `rowData` omitting both `id` and `userId`.
    const rowData: Omit<Contractor, 'id' | 'userId'> = { name: values[headers.indexOf('Name')]?.trim() || '', contact: values[headers.indexOf('Contact')]?.trim() || '', companyName: values[headers.indexOf('CompanyName')]?.trim(), companyAddress: values[headers.indexOf('CompanyAddress')]?.trim(), email: values[headers.indexOf('Email')]?.trim(), comments: values[headers.indexOf('Comments')]?.trim(), }; if (!rowData.name || !rowData.contact) { preview.errors.push({ row: i + 1, message: 'Missing Name or Contact.' }); continue; } preview.validRecords.push(rowData); } setImportPreview(preview); setIsImportModalOpen(true); if(fileInputRef.current) fileInputRef.current.value = ""; };
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Contractors</h2>
                 <div className="flex gap-2">
                    ...
                    {/* Fix: Removed `disabled={isReadOnly}`. */}
                    <button onClick={handleExport} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
