...
const ContractorsScreen: React.FC = () => {
    ...
    const handleSave = (contractorData: Omit<Contractor, 'id' | 'userId'> | Contractor) => { if (isReadOnly) return; if ('id' in contractorData) { updateContractor(contractorData); } else { addContractor(contractorData); } setIsModalOpen(false); setSelectedContractor(undefined); };
    const handleExport = () => { if (isReadOnly) return; const headers = ['Name', 'Contact', 'CompanyName', 'CompanyAddress', 'Email', 'Comments']; const escapeCsvCell = (cellData: string | undefined) => { if (cellData === undefined || cellData === null) return ''; const str = String(cellData); if (str.includes(',') || str.includes('"') || str.includes('\n')) { return `"${str.replace(/"/g, '""')}"`; } return str; }; const rows = contractors.map(c => [c.name, c.contact, c.companyName, c.companyAddress, c.email, c.comments].map(escapeCsvCell).join(',')); const csvContent = [headers.join(','), ...rows].join('\n'); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement('a'); const url = URL.createObjectURL(blob); link.setAttribute('href', url); const date = new Date().toISOString().split('T')[0]; link.setAttribute('download', `pmpr_contractors_${date}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link); };
    const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => { if (isReadOnly) return; const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = (e) => { const text = e.target?.result as string; processCsv(text); }; reader.readAsText(file); };
    ...
    const handleConfirmImport = () => { if (isReadOnly || !importPreview?.validRecords) return; importPreview.validRecords.forEach(contractorData => { addContractor(contractorData); }); alert(`${importPreview.validRecords.length} contractors imported successfully.`); setIsImportModalOpen(false); setImportPreview(null); };
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Contractors</h2>
                 <div className="flex gap-2">
                    <button onClick={() => fileInputRef.current?.click()} disabled={isReadOnly} className="flex items-center gap-2 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><ArrowUpTrayIcon className="w-4 h-4" />Import</button>
